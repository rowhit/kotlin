...
class PropertyReferenceCodegen(
    ...
    val getFunction =
            if (isLocalDelegatedProperty)
                ...
                localVariableDescriptorForReference.type.memberScope.getContributedFunctions(
                        Name.identifier("get"), NoLookupLocation.FROM_BACKEND
                ).first().original
    ...
    private fun generateAccessors() {
        val getFunction = localVariableDescriptorForReference.type.memberScope.getContributedFunctions(Name.identifier("get"), NoLookupLocation.FROM_BACKEND).first()
        val getImpl = getFunction.original.copy(classDescriptor, Modality.OPEN, getFunction.visibility, getFunction.kind, false)
        functionCodegen.generateMethod(JvmDeclarationOrigin.NO_ORIGIN, getImpl, PropertyReferenceGenerationStrategy(true, getFunction, state))

        if (!ReflectionTypes.isNumberedKMutablePropertyType(localVariableDescriptorForReference.type)) return
        val setFunction = localVariableDescriptorForReference.type.memberScope.getContributedFunctions(Name.identifier("set"), NoLookupLocation.FROM_BACKEND).first()
        val setImpl = setFunction.original.copy(classDescriptor, Modality.OPEN, setFunction.visibility, setFunction.kind, false)
        functionCodegen.generateMethod(JvmDeclarationOrigin.NO_ORIGIN, setImpl, PropertyReferenceGenerationStrategy(false, setFunction, state))
    }
    ...
    inner class PropertyReferenceGenerationStrategy(val isGetter: Boolean, val originalFunctionDesc: FunctionDescriptor, state: GenerationState) :
        ...
        override fun doGenerateBody(codegen: ExpressionCodegen, signature: JvmMethodSignature) {
            ...
            if (isGetter) {
                ...
                value.store(StackValue.local(functionDescriptor.valueParameters.lastIndex + 1, OBJECT_TYPE), v)
            }
